...
/** @brief Returns a heap-allocated path for a locally stored message. Caller must free. */
...
int local_msg_save(const char *folder, int uid, const char *content, size_t len) {
    ...
    if (asprintf(&dir, "%s/email-cli/messages/%s", data_base, folder) == -1)
        return -1;
    if (fs_mkdir_p(dir, 0700) != 0) {
        logger_log(LOG_ERROR, "Failed to create local store directory %s", dir);
        return -1;
    }
    ...
    if (!fp) {
        logger_log(LOG_ERROR, "Failed to open local store file for writing: %s", path);
        return -1;
    }

    if (fwrite(content, 1, len, fp) != len) {
        logger_log(LOG_ERROR, "Failed to write local store file: %s", path);
        return -1;
    }

    logger_log(LOG_DEBUG, "Stored UID %d/%s at %s", uid, folder, path);
    return 0;
}
...
int local_hdr_save(const char *folder, int uid, const char *content, size_t len) {
    ...
    if (!data_base) return -1;
    RAII_STRING char *dir = NULL;
    if (asprintf(&dir, "%s/email-cli/headers/%s", data_base, folder) == -1)
        return -1;
    if (fs_mkdir_p(dir, 0700) != 0) {
        logger_log(LOG_ERROR, "Failed to create local header dir %s", dir);
        return -1;
    }
    RAII_STRING char *path = hdr_path(folder, uid);
    if (!path) return -1;
    ...
    logger_log(LOG_DEBUG, "Stored headers UID %d/%s", uid, folder);
    return 0;
}
        ...
        if (!bsearch(&key, sorted, (size_t)keep_count,
            ...
            if (asprintf(&path, "%s/%s", dir, name) != -1) {
                ...
                logger_log(LOG_DEBUG,
                           "Removed stale header: UID %ld in %s", uid, folder);
            }
        }
    }
    free(sorted);
