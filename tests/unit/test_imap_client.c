...
/*
 * Minimal IMAP server child process.
 *
 * Accepts ONE connection, sends `greeting`, then loops reading lines and
 * replies according to the command:
...
static void run_mock_server(int listen_fd,
    ...
    while (1) {
        ...
        if (strstr(buf, "LOGIN")) {
            send(cfd, login_reply, strlen(login_reply), 0);
...
void test_imap_connect_login_ok(void) {
    ...
    if (pid == 0) {
        run_mock_server(lfd, greeting, login_reply);
